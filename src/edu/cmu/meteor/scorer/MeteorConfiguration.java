...
public class MeteorConfiguration {
	...
	private void setDefaults() {
		...
		setEmbeddingsThreshold(Constants.DEFAULT_THRESHOLD_EMBEDDINGS);
		setCharBased(false);
	}
	...
	public void setEmbeddingsURL(URL embeddingsURL) {
		try {
			// This should not ever throw a malformed url exception
			this.embeddingsURL = new URL(embeddingsURL.toString());
		} catch (MalformedURLException ex) {
			System.err.println("Error: word2vec model URL NOT set");
			ex.printStackTrace();
		}
	}

	public void setEmbeddingsThreshold(double embeddingsThreshold) {
		this.embeddingsThreshold = embeddingsThreshold;
	}

	public double getEmbeddingsThreshold() {
		return embeddingsThreshold;
	}
